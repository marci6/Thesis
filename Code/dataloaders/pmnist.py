...
def get(data_path, seed=0, fixed_order=False, pc_valid=0):
    ...
    if not os.path.isdir('/data/pmnist/'):
        os.makedirs('/data/pmnist/')
        # Pre-load
        # MNIST
        mean = (0.1307,)
        std = (0.3081,)
        ...
        for i, r in enumerate(seeds):
            ...
            for s in ['train', 'test']:
                ...
                torch.save(data[i][s]['x'],os.path.join(os.path.expanduser('/data/pmnist'), 'data' + str(r) + s + 'x.bin'))
                torch.save(data[i][s]['y'],os.path.join(os.path.expanduser('/data/pmnist'), 'data' + str(r) + s + 'y.bin'))
        print()
        ...
        for i, r in enumerate(seeds):
            ...
            for s in ['train', 'test']:
                data[i][s] = {'x': [], 'y': []}
                data[i][s]['x'] = torch.load(os.path.join(os.path.expanduser('/data/pmnist'), 'data' + str(r) + s + 'x.bin'))
                data[i][s]['y'] = torch.load(os.path.join(os.path.expanduser('/data/pmnist'), 'data' + str(r) + s + 'y.bin'))
