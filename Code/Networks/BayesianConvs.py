...
class _ConvNd(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, dilation, transposed, output_padding, groups, use_bias, args):
        ...
        # Prior distributions
        self.weight_prior = Prior(self.std1, self.std2, self.pi, self.device).to(self.device)

        if self.use_bias:      
            self.bias_prior = Prior(self.std1, self.std2, self.pi, self.device).to(self.device)
